...
@st.cache
def UNetpredict(image,saveRoot):
    Net = ONNXModelUNet()
    imagef = cv.resize(image, (256, 256))
    img = np.asarray(imagef)
    imageArrayP = np.expand_dims(img, 0).repeat(3, axis=0)
    imageArray = np.expand_dims(img, 0)
    ...
    imageArrayP = np.squeeze(imageArrayP)
    imageArrayP = np.transpose(imageArrayP, (1, 2, 0))
    resultA = cv.addWeighted(imageArrayP, 0.6, predBGR, 0.4, 0).astype(np.uint8)
    # 保存图像
